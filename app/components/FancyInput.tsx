...
const inputSizes = {
  medium: "min-w-[20px] h-10",
  large: "min-w-[20px] h-11"
};

const FancyInput: React.FC<FancyInputProps> = ({ label, size = 'medium', type = 'text', name, value, onChange, ...props }) => {
    ...
    <div className={`flex flex-col items-end gap-6 w-42`}>
      <div className={`relative w-full ${sizeClasses}`}>
        <input
          ...
          name={name}
          {...props}  // Pass all other props to the input
        />
